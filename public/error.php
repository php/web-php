...
require_once __DIR__ . '/../include/errors.inc';
...
if ($URI == 'phpnetsearch.src') {
    status_header(200);
    require_once __DIR__ . '/../include/mozsearch.inc';
    exit;
}
// FIXME: Nuke the old firefox search plugin
if ($URI == 'phpnetimprovedsearch.src') {
    status_header(200);
    require_once __DIR__ . '/../include/mozopensearch.inc';
    exit;
}
...
if (preg_match("!^security/advisories/PHPSA-(\\d+)\\.php$!", $URI, $array)) {
    status_header(200);
    $_GET["id"] = $array[1];
    require_once __DIR__ . '/security/index.php';
    exit;
}
...
if (!in_array($URI, ['mirror-info', 'error', 'mod'], true) &&
    file_exists(ProjectGlobals::getPublicRoot() . "/$URI.php")) {
...
if (isset($uri_aliases[$URI])) {
    ...
    if (file_exists(ProjectGlobals::getPublicRoot() . "/$URI.php")) {
        $target = "/$URI.php";
        if ($queryString !== '') {
