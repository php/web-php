...
function show_types($first_item, $show_any, $var_name) {
	               ...
	               'Function Specific' => 
	                   array (
	                       ...
	                       'Graphics Functions' =>
	                           array('GD Related',
	                                 'SWF Related'),
	                       'Languages/Translation Functions' =>
	...
	foreach ($items as $topic => $subtopics) {
		...
		if (!is_array ($subtopics)) {
			_types_display_entry($subtopics, $selected, 0);
			continue;
		}
		_types_display_entry ($topic, $selected, 0);
		_types_show_subtopics ($subtopics, $selected, 1);
	}
	echo "</select>";
}
