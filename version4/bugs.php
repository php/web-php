...
if (isset($cmd) && $cmd == "Send bug report") {
    ...
    if (Mail($destination, "PHP 4.0 Bug #$cid: $sdesc", $ascii_report, "From: $email")) {
		...
		echo "or update your report at any time.  The URL for your bug report is: <a href=\"http://bugs.php.net/version4/?id=$cid\">";
		echo "http://bugs.php.net/version4/?id=$cid</a>\n";
    } else {
...
#	$fields[] = "php_os as OS";
	...
	if(isset($modify) && $modify=="Edit Bug") {
		...
		if(!$ok) {
			...
			$text = "ID: $id\nUpdated by: $user\nReported By: $eemail\nStatus: $estatus\nBug Type: $ebug_type\nAssigned To: $eassign\nComments:\n\n$ncomment\n";
			$text .= "\nFull Bug description available at: http://bugs.php.net/version4/?id=$id\n";
 ...
 <tr>
  ...
  <td>
   <select name="php_version">
    <option name="4.0b4pl1" selected>4.0 Beta 4 Patch Level 1
