...
<p>After a lengthy QA process, PHP 4.1.0 is <a href="http://www.php.net/downloads.php">finally out</a>!</p>
...
<ul>
 ...
 <li>Revolutionary performance and stability improvements under
  ...
  with us to improve PHP for Windows.</li>
 <li>Versioning support for extensions.  Right now it's barely being
  ...
  Make sure you only use extensions built with PHP 4.1.0.</li>
 <li>Turn-key output compression support</li>
 <li><b>LOTS</b> of fixes and new functions</li>
</ul>

<p>As some of you may notice, this version is quite historic, as it's
...
<p>Following is a description of the new input mechanism.  For a full list of
changes in PHP 4.1.0, see the <a href="ChangeLog-4.php">changelog</a>.</p>

<hr />

<h2>SECURITY:  NEW INPUT MECHANISM</h2>

<p>First and foremost, it's important to stress that regardless of
anything you may read in the following lines, PHP 4.1.0 <b>still supports</b>
the old input mechanisms from older versions.  Old applications
should go on working fine without modification!</p>

<p>Now that we have that behind us, let's move on :)</p>

<p>For various reasons, PHP setups which rely on register_globals 
...
exploitable by things related to this misfeature.</p>

<p>While it is quite possible to write secure code in PHP, we felt that the
...
people to shut it off whenever possible.</p>

<p>To help users build PHP applications with register_globals being off,
we've added several new special variables that can be used instead of the
old global variables.  There are 7 new special arrays:</p>
...
<ul>
 ...
 <li>$_REQUEST - a merge of the GET variables, POST variables and Cookie variables.
            In other words - all the information that is coming from the user,
            and that from a security point of view, cannot be trusted.</li>
 <li>$_SESSION - contains HTTP variables registered by the session module</li>
</ul>

<p>Now, other than the fact that these variables contain this special information,
...
an implicit putenv().</p>

<p>PHP 4.1.0 still defaults to have register_globals set to on.  It's a
...
<p>As of the next semi-major version of PHP, new installations of PHP will
...
then too - you can turn it on if you choose to.</p>

<p>Note:  Some of these arrays had old names, e.g. $HTTP_GET_VARS.  These names
still work, but we encourage users to switch to the new shorter, and
auto-global versions.</p>

<p>Thanks go to Shaun Clowes (shaun@securereality.com.au) for pointing out this
problem and for analyzing it.</p>
